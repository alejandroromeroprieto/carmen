...
def log_prior(theta, normalizer):
    # theta is already dimensional here
    # Flat priors in your parameter ranges
    for val, (low, high) in zip(theta, normalizer.pranges):
        if not (low <= val <= high):
            return -np.inf
    return 0.0  # log(1)

def log_likelihood(theta, normalizer, esm_data, model):
    # Normalize theta -> [0,1] space before cost calculation
    theta_bar = normalizer.normalise(theta)
    cost = calculate_cost_gen_func_cross_experiment(
        theta_bar, normalizer, esm_data, model, "gpp0", "cveg", "gpp"
    )
    return -0.5 * cost

def log_posterior(theta, normalizer, esm_data, model):
    lp = log_prior(theta, normalizer)
    if not np.isfinite(lp):
        return -np.inf
    return lp + log_likelihood(theta, normalizer, esm_data, model)
...
initial_lit_guess_dim = np.array([1, 0.3406, 2.7698, 10, 0, 0, 0, 0.8])
...
for model, experiment_list in model_experiment_dict.items():
    ...
    if "GPP" in fluxes_to_calibrate:
        ...
        NPAR = len(initial_gpp_guess_dim)
        ...
        # Random initial positions of walkers near the initial guess
        pos = initial_gpp_guess_dim + 1e-4 * np.random.randn(nwalkers, NPAR)

        sampler = emcee.EnsembleSampler(
            nwalkers,
            NPAR,
            log_posterior,
            args=(gpp_normaliser, esm_data, model),
        )

        print("Running MCMC for GPP...")
        ...
        for name, mu, med, lo, hi in zip(param_names, means, medians, p05, p95):
            print(f"{name:10s} mean={mu:.3f}, median={med:.3f}, 5–95% interval=({lo:.3f}, {hi:.3f})")

        import corner
        fig = corner.corner(samples, labels=["gpp_t_l","gpp_t_e","gpp_c_l","gpp_c_half","gpp_c_e","gpp_hyst","gpp_c_tan","gpp_fast"])
        fig.savefig("posterior_gpp.png")
        gpp_t_l, gpp_t_e, gpp_c_l, gpp_c_half, gpp_c_e, gpp_hyst, gpp_c_tan, gpp_fast = pstar = medians

        p_bar = gpp_normaliser.normalise(means)
        ...
        # gpp_t_l, gpp_t_e, gpp_c_l, gpp_c_half, gpp_c_e, gpp_hyst, gpp_c_tan, gpp_fast = pstar = gpp_normaliser.inv(p_bar)
    ...
    if "litter" in fluxes_to_calibrate:
        ...
        # p_bar is the normalized solution. Call inv method of gpp_normaliser to get
        # dimensional quantities.
        lit_t_l, lit_t_e, lit_c_l, lit_c_half, lit_c_e, lit_hyst, lit_c_tan, lit_fast = pstar = lit_normalizer.inv(p_bar)
