"""
List of physical and model constants.
"""

# Paths
PARS_DIR = "data/pars/definitive"
SCEN_DIR = "data/scenarios"
OUTPUT_DIR = "output"

GAS_EXCHANGE_COEF = 0.1306  # for the carbon exchange between atmosphere and OML
KELVIN_0 = 273.15  # 0 degrees celsius in kelvin
PPM2GT = 2.1367668  # conversion of ppm to GtC
OCEAN_AREA = 3.55e14  # From Joos paper
OCEAN_FRAC = 0.710369  # Fraction of the Earth covered by ocean
OCEAN_PRESSURE_SEN = 0.0423 # Sensitivity of sea surface partial pressure to temperature
